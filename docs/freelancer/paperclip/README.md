...
Contrato de saida: depois de aprovar, Guardiao consulta `node scripts/freela-crm.mjs whatsapp outbox status --outbox-id [id]`. Se o status mostrar `Pode despachar: sim`, ele chama o Gateway com o mesmo `--outbox-id`. `delivery_pending` exige aguardar `message.ack`; `sent` so vale com ACK forte. O modo sem `--outbox-id` e reservado para operacao assistida em lote, nao para worker.
