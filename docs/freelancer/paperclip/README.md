...
WAHA e o unico transporte operacional de WhatsApp e continua em laboratorio controlado ate os testes de controle fecharem sem placeholder. Ele fica documentado em `docs/freelancer/paperclip/whatsapp-waha-local.md`. Use somente pelo Gateway com `serve-waha-webhook`, `import-waha-event` e `dispatch-approved-outbox --provider waha`; workers nao chamam `/api/sendText`. Enquanto o ACK forte nao chega, a Outbox fica em `delivery_pending`, sem contar como envio concluido. O CRM so considera entregue quando `message.ack` chega com `DEVICE`, `READ` ou `PLAYED`; `message.waiting` marca risco de placeholder e move para handoff. `@lid` continua identidade de leitura e nao deve ser salvo como destino direto na Outbox; telefone real pode virar `@c.us`, mas o Gateway tambem pode usar `@lid` quando ele vier resolvido pelo `check-exists` da WAHA a partir desse telefone real.
