...
- Use `node scripts/freela-ops-doctor.mjs check` quando houver duvida sobre confiabilidade do SQLite, backups ou operacao local. O status operacional pode ser `green`, `yellow` ou `red`. Se `.scratch/ops/reliability-status.json` estiver `red`, pare novas escritas criticas e publique/escale via issue `Ops Health`, documento `reliability-status`, sem dados brutos.
