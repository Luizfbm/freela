...
test("commercial status prioriza handoff aberto antes de acionar worker duplicado", () => {
  const root = makeRoot();
  assert.equal(run(root, ["init"]).status, 0);
  upsertLead(root, {
    canonical_name: "Studio Pronto Redator",
  ...
  const status = run(root, ["commercial", "status", "--date", "2026-06-23"]);
  assert.equal(status.status, 0, status.stderr);
  assert.match(status.stdout, /Prontos para Redator: 1/i);
  assert.match(status.stdout, /Handoffs abertos: 1/i);
  assert.match(status.stdout, /Proximo melhor passo: verificar handoffs abertos e destravar workers\./i);
});
